...
This surfaced a profitable trading strategy for `PEBBLES` but also signaled that the baskets for other assets may also hold useful information.

The `PEBBLES` strategy was constructed as follows:
...
![Round 5 jump-reversal trade visualization](./assets/historical_prices/Round-5/JUMP_REVERSAL_TRADES.png)

The chart above uses the Round 5 trade tape to make the signal easier to see. Prices are direction-normalized in the event study: after a detected jump, a move below zero means the next prints moved back against the jump, which is the reversal the strategy was trying to capture. The bottom-right panel shows a concrete `MICROCHIP_OVAL` example where a `+129` print-to-print jump triggered a sell signal, with the negative beta pulling the expected fair value below the jumped price.

There were several issues with this strategy that should have been taken care of:

- The small position limit meant upside was capped considering how small the moves were
- Big risk of overfitting, since the parameters came from historical data there was a lot of uncertainty about whether it would work in the simulation round
- Risk management was not properly implemented in case of adverse movement
...
![Round 5 strategy component PnL](./assets/historical_prices/Round-5/STRATEGY_COMPONENT_PNL.png)

To understand which modules carried the result, I used `prosperity4btest` from https://github.com/nabayansaha/imc-prosperity-4-backtester  to run each Round 5 strategy component on its own across days 2-4. These numbers are standalone diagnostics rather than additive attribution: in the combined strategy, modules share position limits, overlap on products, and earlier modules can block later ones from placing orders. Even so, the isolated tests made the main sources of edge clear: pair relative value and jump reversal were the largest contributors, followed by bucket-level basket EMA trades.
...
#### Signal-to-Strategy Relationships
